...
resource "aws_glue_trigger" "nightly_aws" {
  name     = "nightly_aws"
  schedule = "cron(0 5 * * ? *)"
  type     = "SCHEDULED"

  actions {
    job_name = aws_glue_job.aws.name
  }
}

resource "aws_glue_trigger" "nightly_BikiniBottomTwitter" {
  name     = "nightly_bikinibottomtwitter"
  schedule = "cron(0 5 * * ? *)"
  type     = "SCHEDULED"

  actions {
    job_name = aws_glue_job.bikinibottomtwitter.name
  }
}

resource "aws_glue_trigger" "nightly_whitepeopletwitter" {
  name     = "nightly_WhitePeopleTwitter"
  schedule = "cron(0 5 * * ? *)"
  type     = "SCHEDULED"

  actions {
    job_name = aws_glue_job.whitepeopletwitter.name
  }
}

### NEW TRIGGERS ###

resource "aws_glue_trigger" "nightly_blackpeopletwitter" {
  name     = "nightly_BlackPeopleTwitter"
  schedule = "cron(0 5 * * ? *)"
  type     = "SCHEDULED"

  actions {
    job_name = aws_glue_job.blackpeopletwitter.name
  }
}
...
resource "aws_glue_job" "blackpeopletwitter" {
  name     = "blackpeopletwitter-curation"
  role_arn = aws_iam_role.glue.arn
  max_retries = 0
  glue_version = "3.0"
  worker_type = "G.1X"
  ...
  command {
    script_location = "s3://${var.s3_bucket_name}/scripts/blackpeopletwitter-curation.py"
  }
...
resource "aws_glue_job" "whitepeopletwitter" {
  name     = "whitepeopletwitter-curation"
  role_arn = aws_iam_role.glue.arn
  max_retries = 0
  glue_version = "3.0"
  worker_type = "G.1X"
  ...
  command {
    script_location = "s3://${var.s3_bucket_name}/scripts/whitepeopletwitter-curation.py"
  }
...
resource "aws_glue_job" "bikinibottomtwitter" {
  name     = "bikinibottomtwitter-curation"
  role_arn = aws_iam_role.glue.arn
  max_retries = 0
  glue_version = "3.0"
  worker_type = "G.1X"
  ...
  command {
    script_location = "s3://${var.s3_bucket_name}/scripts/bikinibottomtwitter-curation.py"
  }
...
resource "aws_glue_job" "aws" {
  ...
  command {
    script_location = "s3://${var.s3_bucket_name}/scripts/aws-curation.py"
  }
